...
async def shutdown(signal, loop):
    # Cleanup tasks tied to the service and shutdown
    logging.info(f"Received exit signal {signal.name}")
    logging.info("Closing database connections.")
    logging.info("Nacking outstanding messages.")

    # Collect all the running tasks and cancel them one-by-one
    tasks = [t for t in asyncio.all_tasks() if t is not asyncio.current_task()]
    [task.cancel() for task in tasks]
    logging.info(f"Cancelled {len(tasks)} outstanding task.")
    # asyncio.gather doesn't return exceptions by default. This may leave the service in weird state. Using reuturn_exceptions makes it return the exceptions.
    await asyncio.gather(*tasks, return_exceptions=True)
    logging.info("Flusing metrices")
    loop.stop()

    """
    In production code we may want to check for exceptions other than asyncio.CancelledError as it may suggest error within the awaitable object itself
    """
