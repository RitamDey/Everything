...
class PoetryProtocol(Protocol):
    ...
    def makeConnection(self, transport):
        # This method is responsible for making the connection, marking the
        # client if they are connected or not and populating the `.transport`
        # class with the appropriate the Transport object
        super().makeConnection(transport)  # Let the parent class do the work
        # Import the reactor and use the `.callLater` function to schedule
        # a cancelation task after 2 minutes via `.transport.looseConnection`
        # which triggers the transport to notify and close the connection
        from twisted.internet import reactor
        self.cancel = reactor.callLater(120, self.transport.loseConnection)
    ...
    def poemRecevied(self, poem):
        ...
        if self.cancle.active():
            self.cancel.cancel()  # Cancel the cancellation task if its active
